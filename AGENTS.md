...
10. **成本意識：迴圈很貴，要設計成「負擔得起」**。一條迴圈動輒 50–200K token、回環三輪 500K–2M —— Loop Engineering 的成敗在**負擔得起**，不是能不能跑。所以全程貫徹：
    ...
    - **fail-fast 不空轉**：停止條件**看收斂**（同條 finding 復現 / 修出新問題就 escalate 換手法；findings 沒變少先歸因「驗證手段變深」還是「修壞了」）、回環圈數是**軟上限＝回報檢查點**（**未修的 P0/P1 不得因圈數收圈**，只剩 P2/P3 才當停損點；auto 無人監管另設**硬性總圈數天花板** `LOOPS_AUTO_MAX_ROUNDS` 預設 6，撞頂 escalate 停下交人、attended 不受影響——見 `references/auto-mode.md`；「未清 P0/P1 不准收圈」另由 pr-gate 閘⑥ 機械化）、**不過早放棄也不無限繞**。
