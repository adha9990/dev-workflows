...
> **2026-07-22 更新（未實作、實作時須對齊；下界於 2026-07-27 由 issue #211 放寬）**：本稿多處寫的「內圈 3 圈上限」已被 issue #161 改成**軟上限**——`iterate` 的圈數到頂只觸發回報，**未修的 P0 不得因圈數收圈**（見 `skills/iterate` §5；P1 不再機械擋收圈，但仍照 actionable 全修的紀律修）。哨兵層自己的外圈輪數計數（第 4 輪起不派發、改 escalate）維持原設計；動工前把「內圈照 iterate 既有 3 圈」一律讀成「照 `iterate` §5 當時的規則」。
>
