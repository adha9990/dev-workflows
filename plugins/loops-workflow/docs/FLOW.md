...
| **機制** | 一次一問（決策點標推薦、記 HYPOTHESIS+CONFIDENCE、should-want 偵測）→ restate + **一次確認** → 寫 `00-clarify.md` → 交棒。**不建 issue / 不釘 DoD / 不動 code** |
...
| **機制** | **一定停**開安全停等級決策點確認（scaffold 是大動作 + 棧定死）→ 合用就跑模板 + `pnpm install` + typecheck/lint/test 驗收 → 接後續（**發散式 explore** 盤 backlog／§1.5 **define** 單一問題）。要別的棧 → 提示自建骨架。（internal skill，僅由 dispatch 路由；也可自然語言請求） |
...
| **技能** | 14 個 skill（SKILL.md 統一骨架） | Skills |
