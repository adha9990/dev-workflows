...
**讀法**：實線往下是 routine（不問你、直接走）；`✋` = **會停下開一個決策點問你**的真決策點（形狀 + 平台映射見 `interaction-adapter.md`）。`iterate` 把問題修完會**回環**（看收斂；圈數預設 3 是**軟上限**＝到頂只回報現況，還有未修的 P0/P1 就繼續修）再驗，全乾淨才完工開 PR。explore 有兩種出口：**收斂式**→plan（選一個方法）、**發散式**→define（把設計空間盤成 issue backlog，開完停下等後續逐步解，不強制續跑）。
