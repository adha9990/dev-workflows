...
| **收尾清理（合併後）** | 刪已合併 PR 分支（`gh pr merge --delete-branch`，遠端/本機只留 `main`+進行中）· 移除 worktree（`git worktree remove`/`prune`）· **loop 暫存不入庫**（草稿/截圖/worktree/`.loops`/`data` 由 `.gitignore` 涵蓋，`git ls-files` 掃一遍） |
