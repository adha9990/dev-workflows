...
| **完工交接物（依類型）** | **修正型**＝一份修正回覆 comment（`comment-policy` §8 版型：工程角度根因/怎麼修/怎麼驗＋客戶角度修正前→後；**不@reviewer**）；**完整迴圈**＝PR 收尾 comment + **explain（`LOOPS_EXPLAIN=1` 才產，未開跳過＋Journal 留痕）**。**`CHECKLIST.md` 一律交人類手動驗收**（verify 產、不吃旗標、explain 沒開也給）；**friction-capture**（Journal 有 `[friction]` 筆記 → 派 subagent 產 plugin 修改提案、主線 `AskUserQuestion` 問是否寫進 plugin，agent 只提案不動 plugin）。follow-up 留當前 issue 不另開。PR body 放 `Closes #issue`、指派 `@me`、與 master 衝突自動合併 |
