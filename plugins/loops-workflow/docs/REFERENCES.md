...
> `references/` 是 loops-workflow 的「知識模組」—— 各階段 skill / agent 不重述共用規範，而是引用這裡。本檔把 **45 份 reference 依功能分類**，一眼看出「哪份在管什麼、誰在用」。
>
...
| `acceptance-review` | 驗收深度：錯題偵測 P0 / 完成度五態 / 端到端 + 持久化 / 證據強弱 | verify（product-contract） |
