...
| `verify-triage` | verify 改動風險 4 級梯判準（SKIP/LIGHT/STANDARD/DEEP）：高風險硬閘清單 / SKIP·LIGHT 護欄 / tangling veto / DEEP §1.6 tripwire bounce 門檻（+ 明文區分 tripwire〔DEEP-only 早退〕vs acceptance-completeness ledger〔tier-independent 出口 gate〕） | verify（§1.4 判級） |
...
| `acceptance-review` | 驗收深度：錯題偵測 P0 / 完成度五態（逐項強制完整 + descoped 處置）/ **完整性 gate（tier-independent、餵 verify §4.5 出口 gate）** / 端到端 + 持久化 / 證據強弱 | verify（product-contract） |
