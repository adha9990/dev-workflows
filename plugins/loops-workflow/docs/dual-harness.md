...
## 操作指南｜量另一個平台的成本能力（capability probe）

成本歸因要能跨平台，前提是先知道**這個平台實際拿得到什麼**。probe 就是在回答這件事：

```
node plugins/loops-workflow/scripts/codex-telemetry.mjs --json
```

它會逐項回報五種能力——每個回合的用量、agent 身分、上層 agent 身分、工具欄位、對話紀錄位置——
**各自量、互不推論**。拿得到 token 不代表拿得到 agent 身分：這兩件事在 runtime 裡由不同機制提供，
用其中一個推另一個，會產出一份看起來完整、實際有一半是猜的報告。
...
|---|---|---|
| 每個回合各自的用量 | `exact` | 直接用 |
| 只有累計計數器 | `estimated` | 相鄰兩次快照相減（`cumulative-delta`）；第一筆之前沒有基準，不算成一個回合；計數器倒退代表換了 session，該段標「沒量到」而不是記成負數或 0 |
| 都沒有 | `not_measured` | 不編數字 |

**目前的狀態（誠實標記）**：這個 repo 還**沒有**在真的 Codex 上跑過 probe，所以能力清單裡的
codex 欄位維持「沒量到」。要把它改成「支援」，必須真的跑一次、把觀測到的欄位名與來源行號填進報告——
**不能**照官方文件描述或程式裡的參數命名反推。沒量過就寫沒量過，是這個 repo 的硬規則（`AGENTS.md` 規則 5）。

不論走哪個模式，產出的事件**欄位集合完全相同**：能力差異只反映在「這個數字量得多準」那一欄，
不會長出第二套結構。否則讀成本報表的人會分不出「兩個平台真的花得不一樣」還是「工具能力不同」。
