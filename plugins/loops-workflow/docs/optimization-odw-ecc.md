...
- **sub-agent 右尺寸化**：cobus「triage pass is cheap；spawn sub-agents **only** when actionable」→ verify 依改動面定最小 reviewer 集（但 **code 改動不縮核心 6 軸**）。
...
| **#8** verify reviewer 右尺寸化（B2） | 把既有 ad-hoc 的「依改動面派 reviewer」寫成明文規則（§1.4）：純文件/設定縮成 2-3 軸，**code 改動硬規定不縮核心 6 軸** | **PR #13** `9b52557` | 省 reviewer fan-out 成本同時用「code 不縮 6 軸」守住成果；codify 既有實踐 |
...
- **verify reviewer 右尺寸化**（cobus #8）：依改動面定最小 reviewer 集、code 不縮核心 6 軸，見 `skills/verify/SKILL.md` §1.4。
...
- `[85%]` docs-only verify 派 2-3 軸、含 code 派核心 6 軸不縮（← 正是 cobus #8 codify 的規則）
