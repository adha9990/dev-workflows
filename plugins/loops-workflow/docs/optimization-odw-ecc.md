...
- **sub-agent 右尺寸化**：cobus「triage pass is cheap；spawn sub-agents **only** when actionable」→ verify 依改動風險定最小 reviewer 集（#8 二分 → **#38 升 4 級風險梯**：含 code 亦可右尺寸化、高風險升 DEEP 加碼）。
...
| **#8** verify reviewer 右尺寸化（B2） | 把既有 ad-hoc 的「依改動面派 reviewer」寫成明文規則（§1.4）：純文件/設定縮成 2-3 軸、code 不縮核心 6 軸（**後由 #38 升 4 級風險梯，code 內部亦分級**） | **PR #13** `9b52557` | codify 既有實踐；省 reviewer fan-out 成本；**#38 接續把 code 也納入 LIGHT/STANDARD/DEEP** |
...
- **verify reviewer 右尺寸化**（cobus #8 → #38）：依改動風險定最小 reviewer 集（#8 二分 → **#38 4 級風險梯**，含 code 可右尺寸化、高風險 DEEP 加碼），見 `skills/verify/SKILL.md` §1.4。
...
- **13 skills、43 references**；文件全同步（FLOW / REFERENCES / README / journaling）且經各 PR 的 docs-devex reviewer 查核一致。
