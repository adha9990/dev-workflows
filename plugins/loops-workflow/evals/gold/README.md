...
| `artifacts/explanation-quality.json` | artifact 文字快照 `[{id, source, text}]`（56 筆本 repo 真實 commit 訊息，self-contained、judge 與 re-run 都讀此份）。 |
| `judge-results-demo.jsonl` | κ demo 的**獨立 judge-fleet 盲標**軌（55 筆，`track=judge-estimate`、`caseId`↔gold `id`）。judge fleet 看不到 gold 標、只套 rubric。 |
...
## κ demo 結果（可重現）
```
node scripts/eval-poll.mjs kappa --records evals/gold/judge-results-demo.jsonl --gold evals/gold/explanation-quality.json
```
→ **κ = 0.845（strong）**、po=0.945、pe=0.647、paired=55、unmatched=0。

**怎麼讀**：gold-annotator（一個 LLM pass，全 56 筆）與 judge-fleet（三個獨立 LLM agent 盲標分段，共 55 筆）對「pass/fail（score≥4）」的一致性。κ 0.845 表示兩組獨立 LLM 套同一 rubric 高度一致——**這證明 (a) pipeline 端到端可跑、(b) rubric 在獨立 LLM 之間應用穩定**；**但不證明 judge 對齊人類判斷**（兩軌皆 LLM）。boundary 分歧確實存在（如 `eac-33f1ccea`/`eac-bf28a8bc`/`eac-75e96c05` gold=pass、judge=fail）使 κ < 1、非 trivial。
