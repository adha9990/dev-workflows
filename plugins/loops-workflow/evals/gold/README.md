...
| `judge-results-demo.jsonl` | κ demo 的**獨立盲標 judge-fleet** 軌（55 筆，`track=judge-estimate`、`caseId`↔gold `id`，3 個 distinct `judgeId`＝`fleet-judge-1/2/3` 對應三段非重疊盲標、可審）。judge 看不到 gold 標、只套 rubric。 |
...
## κ demo 結果（可重現）
從 repo 根目錄跑（路徑 repo-root-relative，對齊 eval-harness）：
```
node plugins/loops-workflow/scripts/eval-poll.mjs kappa \
  --records plugins/loops-workflow/evals/gold/judge-results-demo.jsonl \
  --gold plugins/loops-workflow/evals/gold/explanation-quality.json
```
