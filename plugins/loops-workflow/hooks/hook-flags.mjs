...
// 容易漂移。本檔把「26 個 flag 各自屬於 defaultOn 還是 optIn」與「怎麼判斷開關」都收斂到這一處：
...
// ── 對外契約：26 個 flag 的分類表（值即契約，逐欄釘死）────────────────────────────
export const FLAG_DEFAULTS = {
  // defaultOn（23）：安全防護 / 觀測類，預設啟用，僅字面 '0' 可關閉。
  ...
  LOOPS_PR_BLOCKING_GATE: { defaultOn: true },
  LOOPS_PR_VALIDATION_GATE: { defaultOn: true },
  LOOPS_PR_FOOTPRINT_GATE: { defaultOn: true },
  LOOPS_PR_EFFORT_GATE: { defaultOn: true },
  LOOPS_PR_CONFLICT_GATE: { defaultOn: true },
  LOOPS_MERGE_GUARD: { defaultOn: true },
  LOOPS_PR_OWNER_GUARD: { defaultOn: true },
  LOOPS_TELEMETRY: { defaultOn: true },
  LOOPS_AGENT_TRACE_GATE: { defaultOn: true },
  LOOPS_ARTIFACT_GATE: { defaultOn: true },
  LOOPS_CONTEXT_PACK_GATE: { defaultOn: true },
  LOOPS_HANDOFF_STOP_GATE: { defaultOn: true },
  LOOPS_DECISION_GATE: { defaultOn: true },
  // optIn（3）：會自動執行 repo 控制的命令 / 注入類 / 主動 block 續跑，預設關閉，僅字面 '1' 可開啟。
  LOOPS_STOP_GATE: { defaultOn: false },
  LOOPS_COMPACT_HINT: { defaultOn: false },
  LOOPS_LOOP_DRIVER: { defaultOn: false },
};
