...
const INSTINCT_HEADER = '★ 從過往 loop 學到（instinct，啟發式非統計、僅供參考）：';

// ── 純函式層（無 IO，測試直接 import）─────────────────────────────────────────────

/**
 * 從單一 instinct YAML 文字抽出 {confidence, summary}。
 * 只 regex 抽兩欄（不引 YAML lib）：confidence 取其後數字（無 / 非數字 → 0），
 * summary 取其後整行並 trim（無 → ''）。容錯：任何缺欄 / 空輸入都回預設、絕不丟。
 */
export function parseInstinct(yamlText) {
  const text = String(yamlText);
  const confidenceMatch = text.match(/^\s*confidence:\s*([\d.]+)/m);
  const confidence = confidenceMatch ? safeFloat(confidenceMatch[1]) : 0;
