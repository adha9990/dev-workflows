...
//
// 預期 Red（T1 階段）：hooks/pr-gate.mjs 尚未建立、hooks.json 尚未掛載——本檔全部 P1–P8／EXTRA
// 案例現況皆為紅。不對 pr-gate.mjs 做「靜態具名 import」（家族其他檔案對已存在的 hook 會這樣
...
// 動態 import 只是示範「即使嘗試把它當 ES module 載入也不會讓本檔崩潰」，不對其具名 export
// 斷言——pr-gate.mjs 本身沒有預先約定的具名純函式契約（見檔頭：team lead 給的契約全在
// payload→輸出的行為層級，不是函式簽名層級），下面 P1–P8 全靠 spawnSync 黑箱驗證。
try {
  await import('./pr-gate.mjs');
} catch (e) {
...
console.log(
  `(T1 現況：hooks/pr-gate.mjs 尚未建立、hooks.json 尚未掛載——expect_red ≈ ${total}/${total}；`
  + `實作補齊 T2 後應全數轉綠且不得修改本檔斷言內容)`,
);
