...
repo 未索引：
- **explore / 主迴圈：預設先用 codebase-memory-mcp 的 `index_repository` 對目標 repo 建索引、再用 graph 查** —— **不因「沒 index」就直接退 grep**（graph 的 token 效率與符號精度值回這一次索引成本）。**退 raw `Read` / `Grep` 只在明確例外**：① 探索範圍窄、預期不會在此 repo 反覆查詢（單一 / 少數符號、一次性）；② repo 檔數少到 grep 一次就夠；③ mcp 不可用、或 `index_repository` 失敗；④ 使用者明示不要 index。
