...
> 2. **不寫客套開場**（感謝 / 謝謝 / thanks…）——直接陳述內容（見 §6）。
>
> hook 是保險絲、不是免讀本檔的理由：手貼 comment 也要先套本檔版型。誤擋逃生口：`LOOPS_COMMENT_GUARD=0`。
...
- **「便宜但留債」的選項必須把債標明在代價面**（留下什麼風險、何時會爆——例：保留新舊雙路徑、暫留 shim、退回本該完成的遷移）；**不得預設標 `(Recommended)`（除非使用者已明示接受該債）**——推薦以長期正確性與風險消除為先，「代價小」只在同等正確的選項間當 tie-breaker（AGENTS 規則 10）。若所有可行選項都留債，推薦**長期最接近正確、債最明確可控**的那個，並在每個選項明標各自的債。留債不是「你覺得算不算」的歸類題——**行為回歸**（例：改動會讓某個既有功能悄悄壞掉，而這次選擇不修）也是留債的一型，完整客觀判準與例外邊界見 **AGENTS 規則 10**；「務實 / 先擋住最糟的 / 小步前進」的包裝不豁免。auto 模式自動採推薦選項——這條在 auto 下就是拍板本身。
...
| 術語湯（重寫前） | 白話（重寫後） |
|------|------|
| 「usage_count read model 要維護式 cache（LRU keyed by library、snapshot version = `MAX(op_log.seq)`、`onCommittedOpLog` dirty invalidation）還是現算 + ETag/304（`INDEXED BY` + `CROSS JOIN` + EXPLAIN-guard）？」 | 「『這個標籤被用了幾次』要不要先存起來？<br>**A 每次現算（推薦）**：打開標籤管理才數一次，平常很快、只有剛改完標籤那一下、且庫超大時會稍慢一點；做法簡單，之後真的太慢再加速也不遲。<br>**B 先存起來**：改一次就順手更新存檔，就算庫超大也一直很快；但要多養一套存檔＋自動更新的機制，現在就得維護、比較複雜。」 |
