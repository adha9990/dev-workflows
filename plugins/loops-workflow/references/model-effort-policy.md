...
| `finding-validator` | `sonnet` | `low` | 窄任務 |
| `eval-judge` | `sonnet` | `low` | 窄任務 |

> 分層依據：`sonnet`·`medium` 為廣度審查 / 一般實作的預設；窄任務降 `sonnet`·`low`；罕見高判斷（`referee`）用 `opus`·`high`。兩個 `-deep` 變體 frontmatter 靜態即 `opus`·`high`，但平常不派、僅 Phase 2 高風險改派時才出。
