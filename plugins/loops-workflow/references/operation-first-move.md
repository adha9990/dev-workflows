...
- 由 dispatch（建 loop.md 時）或 goal（起點若是 goal）**依 issue 內容判定**並寫進 loop.md 的 `operation` 欄——**非自動偵測程式**，是人/Claude 讀 issue 判。
...
- 這條規則**只影響 test-author 的紅燈第一步**，不改 build 的紅綠分離架構、不改「test-author 看不到 implementation」原則。
