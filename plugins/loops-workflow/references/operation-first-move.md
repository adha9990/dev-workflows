...
- **主要 writer ＝ dispatch**（建 loop.md 時）：dispatch §2 一律先建 loop.md 才進起點階段，故即使 `issue# → 從 goal 起`，operation 仍由 dispatch 寫。**唯有使用者直接喊 `/loops-workflow:goal`（完全未經 dispatch）這條 bypass 入口，才由 goal 補寫。** **依 issue 內容判定、非自動偵測程式**，是人/Claude 讀 issue 判。
- **讀取端 fail-safe（build）**：build 讀到 loop.md **無 `operation` 欄**（升級前建立的舊 loop）→ 比照 fail-safe 視為 `new-feature`（標準 TDD），不停下。
