...
- ⚠️ **fix 型例外（iterate 起、不經 goal）**：PR 回饋的 fix 型 loop 從 iterate 起、**不經 goal → 無 goal 兜底**，僅 build fail-safe 為最後防線。fix 型最常是修 bug，正是最需要「先寫重現測試」起手式之處 → **dispatch 建 fix 型 loop.md 時務必依 PR/回饋性質寫 operation（多半 `bug-fix`）、別漏**，否則會靜默退成 `new-feature`、流失重現測試規則。
