...
- **只是牴觸了作者已留痕的決定**（alignment comment / `stages/02-plan.md` / PR body 寫明的拍板）—— 整條剔除（不降級），**除非**它本身同時是獨立的正確性 / 安全 / 資料缺陷（就算沒那決定也是 bug）。完整判準見 `references/personas/finding-author-decision-rule.md`。
