...
|---|---|---|
| Merge-readiness（合併安全） | `verify`：6 reviewer fan-out（product / architecture / security / performance / code-quality / tests）+ 條件式 reviewer + `finding-validator` 二輪 | **完全等價**。verify 就是 merge-review 引擎本身，連 finding 驗證、P0–P3 分級、Ready/Not ready 收斂都有 |
