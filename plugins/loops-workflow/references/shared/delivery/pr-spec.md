...
複製下面骨架填空、刪掉不適用的段。**這是給人讀的**：先講「做什麼、為什麼」，術語能避就避、非用不可就**當場一句白話解釋**，用具體例子（回傳範例、前後對照）勝過抽象描述 —— user-facing / UI 改動優先用下方「情境對照」表把改前改後攤成一眼可比；機制圖（mermaid）只在真的幫理解時才放。工程流水帳留給 commit / `.loops`，別塞進 body。
...
## 情境對照（before → after）（user-facing / UI 改動強烈建議放；純內部 / 無使用者可感差異就刪）
<用表格列「使用者實際會遇到的情境 × before / after」，一眼看出改了什麼體驗 —— 比純文字條列好讀。每列一個具體情境，before 寫改前會遇到什麼、after 寫改後變怎樣；把邊角情境也放進來（錯誤 / 失敗、空狀態 / 零資料、權限不足、慢網、鍵盤 / 輔具…），別只列 happy path。>

| 情境 | before | after |
|------|--------|-------|
| <使用者做的某件事> | <改前的體驗> | <改後的體驗> |
| <某個邊角情境（失敗 / 空 / 無權限…）> | <改前> | <改後> |
...
- 送出前對外內容（PR body / 回覆）先寫 tmp 草稿給使用者校稿（見 `references/shared/delivery/comment-policy.md`），確認才 post。
