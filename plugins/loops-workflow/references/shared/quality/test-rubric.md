...
**適用範圍（合併前 / 後分流）**：本節管**已出貨** bug——以 GitHub issue 回報、修復走 bug-fix 迴圈的那些；其迴歸測試是跨 session 的唯一記憶（後續重構的 agent 不記得踩過的雷），**每 bug 恰留一條**、不可砍。**in-loop bug**（本迴圈內、PR 未合併前發現並修掉的——錯的 code 從未出貨）**不適用留檔要求**：其迴歸測試視為鷹架，於 iterate 完工收尾的裁測 pass 處理（保留集見 §10 判必要）。
...
前九節防「測不夠 / 假綠」；本節防反向：**測試增量要與行為增量成比例**。測太多同樣是缺陷——維護面積、跑套時間、審查負擔都跟著漲，而多出來的綠燈沒有多守住任何行為。

> **本節是最後一道稽核，不是唯一的控制點**（也不是第一個）。過量與重複的控制順序是：
> ① **plan 的 evidence portfolio**（`references/stages/evidence-portfolio.md`：每個 behavior 恰一份主證據、第二層要寫得出 `distinct_risk`、`new_test=true` 要有理由）→
> ② **build 的路徑選擇**（低風險 slice 不新增測試、test-author 可回 `NO_NEW_TEST_REQUIRED`）→
> ③ **verify 步驟 0 的 footprint 對帳**（新測試沒理由 / 重複證據沒 `distinct_risk` 直接擋）→
> ④ **本節的收尾裁測**。走到 ④ 時多數重複應已不存在；它撿的是前三道漏掉的鷹架與 in-loop 迴歸。

**檔案落點**

- 新測試的**預設落點是既有測試檔加 case**；只有變更引入了新模組（新檔案有自己的職責）才新開測試檔。
- **密度基準＝專案內同類功能的既有測試**：動手前先找一個同類既有功能的測試當基準（檔數 / 條數 / 相對功能碼的比例），新功能對齊它、不自成一格。
- **不為新功能改動共用機制的既有測試檔**（dispatcher / router / 共用 hook 的測試）：機制已有自己的測試，新 consumer 不重測機制本身。

**判多餘（抓到即 finding，六型）**

1. 按動作 / 狀態切分的專屬測試檔家族（一個功能開 `X.hotkey` / `X.command` / `X.tooltip` / `X.a11y` 一窩）——應併回既有檔。
2. 共用機制的「自家切片」重測（為新 consumer 把機制本身再測一遍）。
3. 純接線 / metadata 斷言（測註冊表內容＝測常數：改一行設定要陪改一行測試，守不住任何行為）。
4. 同一路徑的排列組合變體逐條測（開 / 關 / 再開 / 第二下——同路徑只測代表條）。
5. 同一行為跨層重複覆蓋（hook 層＋component 層＋full-App 層驗同一件事——取最低有效層級，見 §6）。
6. 髒 gate：assertions 全綠但有 unhandled errors / exit code ≠ 0 的測試——這種覆蓋是**負資產**，不是安全網。

**判必要（裁減時砍到只剩這些仍不可再砍）**

1. 標題功能的正向核心路徑 ×1：走真實鏈路（不是 spy / private helper），斷言使用者可觀察的結果（§9）。
2. 本次變更**特有**的機制路徑 ×1——功能真的引入特別狀態才配獨立 case。
3. 新功能 × 既有狀態的交互守衛：在狀態 A 被擋、狀態 B 仍通——**同一測試先擋後通**，防假綠。
4. 輸入邊界 guard（修飾鍵 / 非主鍵 / IME 一類的誤觸防護）。
5. **已出貨（issue 回報）bug** 各一條迴歸（過 §7 四問）；in-loop bug 的迴歸不在保留集（§7 分流——收尾裁測砍）。
6. 專案慣例明文要求的軸（i18n / a11y / logging 等專案 `AGENTS.md` 點名者）。

**程序**

- **量級 sanity check（訊號，不是硬閘）**：**新增測試行數 > 新增功能行數（>1:1）＝過度訊號**——口徑是 **PR 增量**（`git diff --numstat` 對 base 分測試檔 / 功能檔加總；測試檔判定依專案慣例 `*.test.*` / `*.spec.*` / tests 目錄），**不逐檔算**（逐檔會誤傷測試合理多於 code 的純邏輯模組）。超標動作：**先按判多餘六型裁**；裁完仍超才留痕說明理由，不為湊數字砍判必要 1–2、也不默默交出去。**比例本身永遠不是擋 PR 的理由**（`scripts/diff-footprint.mjs` 對它只出 warning）——正當的測試不該被 ratio 擋掉；會擋的是「新測試沒有理由 / 重複證據沒有 `distinct_risk`」。
- **裁減時點**：iterate 完工收尾的裁測 pass（verify 乾淨後、交 PR 前）——時點與流程歸 `skills/iterate` §6，本節只管門檻與留 / 砍判準。
- **裁減下限**：標題功能的核心 gate（判必要 1–2）不可砍——砍過頭與測太多同樣是 finding。
- 無自動化守衛的驗收行為必須在 PR body 誠實列出、指明由何種驗證覆蓋（實跑 checklist / manual，§8）。
