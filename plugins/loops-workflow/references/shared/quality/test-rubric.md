...
- **量級 sanity check**：**新增測試行數 > 新增功能行數（>1:1）＝過度訊號**——口徑是 **PR 增量**（`git diff --numstat` 對 base 分測試檔 / 功能檔加總；測試檔判定依專案慣例 `*.test.*` / `*.spec.*` / tests 目錄），**不逐檔算**（逐檔會誤傷測試合理多於 code 的純邏輯模組）。超標動作：**先按判多餘六型裁**；裁完仍超才留痕說明理由，不為湊數字砍判必要 1–2、也不默默交出去。
