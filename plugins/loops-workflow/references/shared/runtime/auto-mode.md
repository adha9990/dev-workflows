...
4. **verify 判 `Not ready`、iterate 回環碰到圈數軟上限**（接 `iterate` §5）—— 分三種走法，**圈數本身不是停止修正的理由**：
   - **還有未修的 P0/P1** → **不停、繼續修**（把現況：未清 P0/P1 逐條 + findings 軌跡 + 歸因 + 下一圈換什麼手法，寫進 `loop.md` Journal ＋ chat 摘要後續修；auto ≠ 靜默）。P0 另依 #5 一律停。
     - **但 auto 有絕對上界：硬性總圈數天花板**。回環總圈數撞到 `LOOPS_AUTO_MAX_ROUNDS`（環境變數，未設＝**6**；與軟上限**同一個持續累加計數**）時，**即使還有未修 P0/P1 也要 escalate 停下回報現況（未清逐條 + 每圈換過什麼手法）、交使用者接手** —— 不自動收圈、不自動帶過。這是給**無人監管 auto** 的絕對上界，防「修一條 P1 揭露新 P1、無界繞圈燒 token」（實測 P1 序列 3→1→5→2→4→1…）。**attended（closed）模式不受此天花板影響**（它每碰軟上限都已停下讓人選，人本來就是天花板）。
   - **已無 P0/P1、只剩 P2/P3** → **停下問**（沿用既有上限停損語意：收圈 / 授權再繞 / 記 out-of-scope 由使用者選）。
   - **「知情帶著未修的 P0/P1 收圈進 PR」永遠是硬煞車** → auto **不得**自動選此項，一定停下讓使用者拍板（放行已知缺陷是使用者的 scope 決策，同 #6 的知情 + 留痕精神）。此條由 `hooks/pr-gate.mjs` 閘⑥ 機械化：auto 模式下即使 `.loops/<slug>/blocking-waiver.md` 存在也**不認**（waiver 只 attended 生效），最近一輪 verify 仍有未清 P0/P1 時 `gh pr create`/`ready` 一律 deny。
5. 任何 reviewer 報出 **P0**。
...
| 安全停點 | 全部（分類模糊 / 危險 / P0 / 規格不清 / 回饋反轉 AC / 帶未修 P0/P1 收圈） | 危險 / 失敗 / P0 / 規格模糊 / 回饋反轉已寫定 AC / **帶未修 P0/P1 收圈**（此項 auto 也不自動帶過）/ **撞硬性總圈數天花板 `LOOPS_AUTO_MAX_ROUNDS`（預設 6）** |
| 核准點 | 每個決策一次 | 計畫拍板一次 |
