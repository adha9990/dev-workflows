...
1. **同回合派 N 個異質 judge**（建議 N=2–3、**不同模型家族**）：每個都派 `agents/eval/eval-judge.md`，prompt 給 rubric 絕對路徑（`references/personas/eval-judge-rubric.md`）+ artifact + 契約。**複用 verify 反偏誤**：
...
2. **捕每個 judge 的 verdict**，寫成 `verdicts.jsonl`，每行一個 judge：
   ```jsonc
   { "judgeId": "judge-1", "model": "<該 judge 實際跑的 model id，依 capability tier 決定，見 capability-registry.json>", "output": "<該 judge 的 raw verdict 文字>" }
   ```
