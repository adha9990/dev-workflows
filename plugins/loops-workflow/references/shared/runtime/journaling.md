...
> | `LOOPS_PR_BLOCKING_GATE` | 開（#188） | loop 分支上 `gh pr create`／`ready` 前查「最近一輪 verify 仍有未修 P0/P1」（讀 `stages/04-verify.md` 末尾機械 marker）→ 擋，機械化 iterate §5「收圈硬條件＝P0/P1 清零」；知情豁免＝非空 `.loops/<slug>/blocking-waiver.md`（**僅 attended 生效、auto 一律不認**，對齊 auto 硬煞車 #4）；marker 缺／無法解析 fail-open。獨立 flag（純讀檔、可單獨關） |
...
> | `LOOPS_AUTO_MAX_ROUNDS`（skill 層 env、非 hook；#188） | **6**（可設定） | auto 模式回環的硬性總圈數天花板——撞頂即使有未修 P0/P1 也 escalate 停下交人（防無人監管無界繞圈燒 token）。**只約束 auto、attended 不受影響**；由 `iterate` §5 讀取執行（非 hook 機械計數，同軟上限本質）——詳 `auto-mode.md` 硬煞車 #4 |
>
