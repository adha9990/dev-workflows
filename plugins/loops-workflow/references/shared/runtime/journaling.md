...
└── deliverables/           # loop 的收尾 / 驗收產出（無編號）
    ├── explain.md          # 理解包（實作導讀 + ownership 自測 + 設計 recap）〔standard／deep 檔位〕
    ├── checklist.md        # 合併前手動驗證 + 已知取捨確認清單〔standard／deep 檔位〕
    ├── cost.md             # 成本 / 規模輪廓（展開 outcome 度量）〔standard／deep 檔位〕
    ├── delivery-note.md    # 上面三份合成一份〔direct 檔位；與上面三份互斥〕
...
- **`loop.md` 留在 loop 根、且是 generated**（不進子資料夾）——它是人的 resume 入口，內容由 `scripts/loop-snapshot.mjs` 從 `events.jsonl` 重生：欄位表 + 仍擋著完工的清單 + 未完成任務 + **最多 12 筆**最近事件（高訊號優先，`toolrun`/`note` 只在額度有剩時才佔位）。**手改沒有意義**（下次重生就蓋掉）；要改狀態就 append 事件。
...
- **context pack 依「當前階段 × 變更範圍 × token 預算」組**（`scripts/context-pack.mjs`），不再整包重讀：預算是**硬上限**，但「仍擋著完工的」（未修 P0/P1、沒過的閘、未決決策）是**受保護區段、預算再緊也不丟**；塞不下時標 `overBudget` 並逐條列出沒放進來的東西（不做靜默截斷）。
- **`blocking-waiver.md`（選用、留 loop 根）**：只在**使用者知情決定帶著未修 P0 進 PR** 時才寫（見 `skills/iterate` §5 知情豁免）——寫明豁免哪幾條 P0 + 理由，是 `pr-gate` 閘⑥（`LOOPS_PR_BLOCKING_GATE`）認可的機械留痕（非空才算、**僅 attended 生效、auto 不認**）。**只剩 P1／P2／P3 時不需要這份檔**（閘⑥ 不擋）——但「帶著未修的 P1 收圈」仍要使用者拍板並同步 issue / PR 留痕。屬審計 artifact、非臨時 scratch，隨 `.loops/` gitignore、收尾清理時不當 scratch 刪（同 `real-run/`）。
- **`stages/`**：goal/explore/plan/build/verify 各寫自己那份 `NN-<stage>.md`（帶編號＝流程順序、可排序）。
- **`deliverables/`**：放兩類收尾產出——①**完工文件**：iterate 完工才產、**無編號**、修正型不產；**份數依投入檔位**（`AGENTS.md` 規則 25）——`standard`／`deep` 產三份（`explain.md`＋`checklist.md`＋`cost.md`），`direct` 改產一份合併的 `delivery-note.md`（三段：實作導讀／合併前手動驗證／成本輪廓）。**兩者互斥、不並存**；縮的是份數不是內容（見 `skills/iterate` §6）；②**`real-run/` 真機驗證證據**：驗證改動時把真機截圖（或非視覺 loop 的非空 `no-ui.md`）存這裡，**在開 PR 之前**就產（不是完工才產），是 pr-gate 閘④ 查驗的 receipt（`LOOPS_PR_REALRUN_GATE`）。兩類都隨 `.loops/` gitignore、不入庫。`real-run/` 是**驗證證據、非臨時 scratch**，收尾清理時不當截圖 scratch 一併刪（見 `skills/iterate` 收尾清理）。
...
| token≈ | 估算：`≈120K(中)est` / `≈?(低)est`；實測：`≈127K(measured) ｜ $0.38` | **兩種來源、單一欄**：①**無 hook 實測** → 粗估或級距、必帶 `est`（級距：低 <100K／中 100–500K／高 >500K；無從估寫 `≈?(<級距>)est`）。②**有 cost-tracker 實測**（<!-- adapter-projection -->`.loops/.metrics/costs.jsonl`<!-- /adapter-projection --> 有本 session 末行，見下）→ 改寫 `≈<總>K(measured) ｜ $<usd>`，總＝input+output+cache_creation+cache_read。**注意 `measured` 仍是「依 API 回報 usage 的估算」、非帳單權威**，且有兩個**方向相反**的偏差：<!-- adapter-projection -->寫死 rate table 無法表達 Opus >200K／1h-cache 2× 級距<!-- /adapter-projection --> → 偏**低估**；跨 `--resume` 對整份 transcript 重複加總 → 偏**高估**。<!-- adapter-projection -->Claude Code 不保證對 agent 暴露 per-turn token<!-- /adapter-projection -->，故兩種來源都標明非精準（規則 5 Metric-Honesty）。 |
...
> | `LOOPS_PR_BLOCKING_GATE` | 開（#188、下界 #211 放寬） | loop 分支上 `gh pr create`／`ready` 前查「最近一輪 verify 仍有未修 **P0**」（讀 `stages/04-verify.md` 末尾機械 marker 的 `p0` 欄位）→ 擋，機械化 iterate §5「收圈的機械下界＝P0 清零」；`p1` 不參與判定；知情豁免＝非空 `.loops/<slug>/blocking-waiver.md`（**僅 attended 生效、auto 一律不認**，對齊 auto 硬煞車 #4）；marker 缺／無法解析 fail-open。獨立 flag（純讀檔、可單獨關） |
> | `LOOPS_PR_VALIDATION_GATE` | 開（#209） | loop 分支上 `gh pr create`／`ready` 前查「這一輪有候選 blocking finding，卻一條都沒過 `finding-validator` 二輪確認」（同一個 marker 的 `findings=`／`validated=`）→ 擋，機械化 verify §3「候選 blocking 全數二輪確認」；**不認 waiver**（二輪沒跑＝風險未經評估，沒有東西可以知情接受）；兩欄缺／`findings=0` fail-open。獨立 flag（純讀檔、可單獨關） |
> | `LOOPS_PR_FOOTPRINT_GATE` | 開（#215） | loop 分支上 `gh pr create`／`ready` 前讀 `stages/04-verify.md` 的 `<!-- loops-footprint status=… -->` marker，`status=blocked` → 擋（範圍外施工／新測試沒 `new_test_reason`／重複證據沒 `distinct_risk`／超出 budget 沒 `budget_overrun_reason`）；`warn`（比例超標）與 marker 缺席一律放行。機械化 `AGENTS.md` 規則 17「未說明的 footprint drift」。獨立 flag（純讀檔、可單獨關） |
> | `LOOPS_PR_EFFORT_GATE` | 開（#222） | loop 分支上 `gh pr create`／`ready` 前讀 `stages/04-verify.md` 的 `<!-- loops-effort profile=… floor=… -->` marker，`floor=violated` → 擋（宣稱走 `direct` 檔位、實際改動卻碰高風險硬閘路徑）；`floor=ok`、`highrisk=unknown` 與 marker 缺席一律放行。機械化 `AGENTS.md` 規則 25「檔位是預算、不是豁免」的地板。獨立 flag（純讀檔、可單獨關） |
...
> | `LOOPS_AUTO_MAX_ROUNDS`（skill 層 env、非 hook；#188） | **6**（可設定） | auto 模式回環的硬性總圈數天花板——撞頂即使有未修 P0 也 escalate 停下交人（防無人監管無界繞圈燒 token）。**只約束 auto、attended 不受影響**；由 `iterate` §5 讀取執行（非 hook 機械計數，同軟上限本質）——詳 `auto-mode.md` 硬煞車 #4 |
>
> **觀測 hook（#15；出錯一律 no-op exit 0、永不擋路；預設值逐列標示——#87 起 cost-tracker 預設開、compact-hint 維持 opt-in）**：
> - **`LOOPS_COST_TRACKER`**（cost-tracker，Stop hook；**預設開（#87）——僅字面 `'0'` 關**，顯式 `'1'` 向後相容；僅 cwd 可解析出含 `.loops/` 的主 repo 才寫入、與是否正在跑 loop 無關）：每個 assistant 回應結束把該 session 累計 usage append 一行到 **主 repo**<!-- adapter-projection --> `.loops/.metrics/costs.jsonl`<!-- /adapter-projection -->（per-session **取該 session_id 最後一行**）。**落點錨定（P2）**：不看 cwd 有無 `.loops/`，而是把 cwd 解析成主 repo 根（worktree cwd 也寫回主 repo，對齊 §規則 9 的 `.loops` 錨定）——修好「worktree session 成本寫進 worktree `.loops`、會被清 / 分裂」。每行含 session 累計 **＋ `by_stage` 逐 loop-stage 拆解**（goal/explore/plan/build/verify/iterate…各自 token + `cost_usd`）。**子代理歸戶（P1，schema 3）**：額外掃 `<transcript>/<session>/subagents/agent-*.jsonl`，依角色（reviewer→verify / test·impl-author→build / design→plan / exploring→explore / 其餘→other-subagent）歸到對應 stage，掛在 `by_stage[].subagent`＋頂層 `subagents` 聚合＋`total_cost_usd`（主線+子代理）——**補上 verify / iterate 等幾乎全是 fan-out 子代理的階段成本**（主 transcript 本來看不到，故舊版 verify 常顯示接近 0）。無子代理則維持 schema 2。收尾若該檔有本 session 行→實測分支，否則（未開 flag／無此檔）→估算分支。footprint：主 repo<!-- adapter-projection --> `.loops/.metrics/costs.jsonl`<!-- /adapter-projection -->（已被 `.loops/*` gitignore）。**限制**：① `by_stage` 主線部分按 Skill 邊界切，最後一個 stage 之後的收尾雜項會續記在該 stage 名下；② 子代理歸角色靠 prompt 關鍵字，無法辨識者落 `other-subagent`（不遺失、只是未細分）；③ per-turn 加總 `cache_read` 會高估絕對值（分佈 / output tokens 較可信）。
> - **`LOOPS_COMPACT_HINT=1`**（suggest-compact，<!-- adapter-projection -->`PreToolUse` matcher `Edit|Write`<!-- /adapter-projection -->；**維持 opt-in（#87 決策：非已踩過坑對治、注入有輕噪音、價值中性）**）：估算真實 context 大小（transcript 最新 usage）跨門檻（~250k、之後每 +60k）時，在 Edit/Write 前注入一句「可考慮 `/compact`」**估算**提醒（**不阻擋**工具）；防洗版 state 落 `os.tmpdir()/loops-compact-<session>.json`、14 天 TTL。footprint：tmp state 檔。
> - **read-accumulator**（<!-- adapter-projection -->`PostToolUse` matcher `Read`<!-- /adapter-projection -->；**非 flag——隨 `LOOPS_COMMENT_GUARD`〔其唯一消費端＝outbound-comment-guard 的 read-gate〕開關，#131**）：純記錄，不做任何 `.loops/` 前置判斷（讀規範檔這件事跟目標 repo 是不是 loops 專案無關）。命中兩份受管規範檔之一（`comment-policy.md` / `outbound-templates.md`，basename **精確**比對、大小寫不敏感、`/` 與 `\` 皆視為分隔符）→ 去重累積進本 session 的 state 檔；其餘檔案不記、不建檔。footprint：`os.tmpdir()/loops-reads-<session>.json`。**已知限制**：Read 落盤與後續 Bash 對外指令之間沒有跨行程的嚴格排序保證——理論上兩者「幾乎同時」發生時有極短暫時序落差的可能，但 Claude 的操作序一律是先讀檔才照著寫 comment，實務上落盤必然先於後續呼叫；即便真的踩到，read-gate 頂多多擋一次（要求重讀一次）而不會誤放行——**最壞情況是惱人，不是有害**。
>
...
> **命令類 guard（<!-- adapter-projection -->攔 shell 指令的 `PreToolUse`<!-- /adapter-projection -->）matcher 一律 `Bash|PowerShell`**——Windows 主 shell 是 PowerShell、只掛 Bash 等於半失效（#130）。
> - **`LOOPS_STOP_GATE=1`**（stop-gate，<!-- adapter-projection -->Stop hook + edit-accumulator `PostToolUse`<!-- /adapter-projection -->；**維持 opt-in（#87 決策：預設開＝自動執行任意 repo 的 gate.config.json 命令，#17 security review 明點 RCE 面、信任不可機械判定）**）：**本回合有改檔**時（<!-- adapter-projection -->`PostToolUse` accumulator 記錄<!-- /adapter-projection -->）於 Stop 自動跑既有 `loops-quality-gate.mjs --gates type,lint`，**只有紅燈才把摘要注入 context**（綠靜默、不阻擋）。需 cwd 有 `.loops/gate.config.json`。**發現性提示（#87）**：flag 未開但偵測到 gate.config.json 存在 → per-session 一次注入一行提示（含 `LOOPS_STOP_GATE=1` 與「信任 repo 才開」語；state 檔 `os.tmpdir()/loops-stop-gate-hint-<session>.json`）。footprint：`os.tmpdir()/loops-edits-<session>.json` 暫存＋提示 state 檔。
...
> - **`LOOPS_CONFIG_PROTECTION`**（config-protection，<!-- adapter-projection -->`PreToolUse` matcher `Write|Edit|MultiEdit`<!-- /adapter-projection -->；**預設開（#87）且 loops-scoped**——未設 env 時僅 `payload.cwd` 下存在 `.loops/` 才生效（作弊風險集中於 loops 執行、日常編輯零外溢）；顯式 `'1'`＝全域生效（既有行為）；僅字面 `'0'` 關）：偵測對既有 linter/formatter 設定檔（eslint/prettier/biome/ruff…）的**修改**→ <!-- adapter-projection -->`permissionDecision:"deny"`<!-- /adapter-projection --> 擋下並提示「修 code 別弱化設定；設 `LOOPS_CONFIG_PROTECTION=0` 可關」；**新建**設定檔放行、非設定檔放行。出錯 **fail-open（放行）**。footprint：無持久檔。
...
> - **`LOOPS_PATH_CONTAINMENT`**（loops-path-guard，<!-- adapter-projection -->`PreToolUse` matcher `Write|Edit|MultiEdit`<!-- /adapter-projection -->；**預設開（#85，plugin 唯一 opt-out 介入 hook）——只有字面 `'0'` 會關**，`false`/空字串/未設一律維持啟用）：Write/Edit/MultiEdit 目標路徑落在 `.claude/worktrees/**/.loops/**`（詞法判定：resolve 收合 `..`、大小寫折疊、段完全相等比對）→ <!-- adapter-projection -->`permissionDecision:"deny"`<!-- /adapter-projection -->＋指引正確落點 `$LOOPS_ROOT/.loops/<slug>/`——把 AGENTS 規則 9「.loops 嚴禁寫進 worktree」（已踩過、毀 audit trail）機械化。出錯 **fail-open（放行）**。已知限制：不解析 symlink（熱路徑零 I/O 取捨）。footprint：無持久檔。
...
> - **`LOOPS_WORKTREE_GUARD`**（worktree-guard，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；loops-path-guard 的姊妹規則，那管 `.loops` 落點、本 hook 管 code 落點）：Bash 指令是對**已建 loop**（cwd 祖先存在 `.loops/<slug>/loop.md`，往上最多 12 層）的 branch 做 `git checkout -b <slug>` / `git switch -c <slug>`，且 cwd **不在** worktree（`.claude/worktrees/` 段完全相等比對）→ <!-- adapter-projection -->`permissionDecision:"deny"`<!-- /adapter-projection -->，導向 `git worktree add .claude/worktrees/<slug> -b <slug> <base>`，並註明 session／harness 的「work in place」設定不豁免本條。已在 worktree／非 loop branch／`git branch <name>`（只建不切）一律放行；確需繞過設 `LOOPS_WORKTREE_GUARD=0`。出錯 **fail-open（放行）**。把 AGENTS 規則 9「主 checkout 不 `checkout -b` loop branch」（已踩過坑：文字擋不住合理化）機械化。footprint：無持久檔。已知限制：只攔 Bash/PowerShell 工具呼叫的 git 指令（正則解析、不跨 `; & |` 邊界），不攔其他路徑建立 branch 的方式。
> - **`LOOPS_COMMENT_GUARD`**（outbound-comment-guard，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#131 v2**）：**覆蓋面**從「只管貼 comment」擴成五型對外發訊息指令——`classifyOutboundCommand` 分類成 `comment`（`gh pr/issue comment` 或帶 body 的 `gh api .../comments`）／`issue-create`／`pr-create`／`issue-edit`／`pr-edit`（後四型皆要求帶 body 參數才算，純改 label 等不受管）；非受管指令一律放行。命中受管指令依序過兩關：① **read-gate**——`payload.session_id` 存在時，查 `hooks/read-accumulator.mjs` 記錄的本 session 已讀 state：`comment` 型要讀過 `comment-policy.md`、其餘四型要讀過 `outbound-templates.md`，沒讀過 → <!-- adapter-projection -->`permissionDecision:"deny"`<!-- /adapter-projection -->、指路去讀對應規範檔（絕對路徑、`import.meta.url` 推導）；**缺 `session_id`（舊呼叫形態／smoke）一律 fail-open 放行此關**，不影響下面的機械規則。② **機械規則**——抽出 body（inline `--body`/`-b`/`-F body=`，或檔案形式 `--body-file`/`-F body=@file` 讀檔）後跑 `findOutboundViolations`（**@ 點名人**排除 `@me`／scoped-package／email、**開頭客套**感謝／謝謝／多謝／thank you／thanks／thx——現在對全部五型都管，不只 comment）＋ `findFormatViolations`（**#131 新增三條**：去 code 後 prose 引用 `.loops/` 路徑或裸 `stages/0N-*.md` 檔名；raw body 含 U+FFFD 亂碼字元；去 code＋去 URL 後 prose ≥120 字元且 CJK 字數 <10〔疑似整段技術英文未轉譯，CJK≥10 一律放行不誤擋〕），任一命中 → deny 逐條列出違規原因。乾淨／抽不到 body 一律放行；確需繞過設 `LOOPS_COMMENT_GUARD=0`（同時關掉 read-accumulator 的記錄）。出錯 **fail-open（放行）**。把 comment-policy §0「不外洩 `.loops/` 路徑」／§6「不寫客套」／§8「不 @ 點名 reviewer」、以及「送出前先讀規範」（已出過包：手貼 comment 沒走 outbound 流程就沒載規則、整條漏掉）機械化。footprint：本身無持久檔——讀（不寫）read-accumulator 的 state 檔 `os.tmpdir()/loops-reads-<session>.json`。已知限制：只認得 `gh pr/issue comment`、`gh api .../comments`、`gh issue/pr create`、`gh issue/pr edit`（皆帶 body 參數）這幾種指令形態；@ 點名偵測靠正則（GitHub handle 英數+連字號、1–39 字），非涵蓋所有 edge case；body 若以 shell 變數／指令替換（`"$VAR"`／`"$(cmd)"`）組成，guard 檢查的是替換前的字面文字、不是展開後的實際內容；read-gate 與 read-accumulator 的落盤時序見上方 read-accumulator 條目——**最壞情況是惱人（多擋一次要求重讀），不是有害（不會誤放行）**。
> - **`LOOPS_PR_GATE`**（pr-gate，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#132**）：只在偵測到 `gh pr create` 且**當前處於某個已建 loop 的分支**時生效。**分支判定**兩段式、零 spawn `git`（皆詞法讀檔）：①cwd 路徑含 `.claude/worktrees/<slug>` 段 → slug（重用 worktree-guard 的 `extractWorktreeSlug`）；②否則讀 cwd 的 `.git`（檔案形 `gitdir: <path>` 指標、改讀該路徑下 `HEAD`，或目錄形直接讀 `.git/HEAD`）取 `ref: refs/heads/<branch>` → slug；裸 SHA（detached HEAD，無 `ref:` 前綴）判不出、放行。取得 slug 候選後反查 `.loops/<slug>/loop.md` 是否存在（重用 `findLoopRoot`，祖先上溯 ≤12 層——worktree cwd 剝 `.claude/worktrees/<slug>` 後綴到主根只需 3 層，天然在界內，不必另維護捷徑）確認「是已建 loop」，否則放行。命中後依序過**三閘**、任一不過即 deny、不聚合、後面的閘不再檢查：①`stages/04-verify.md` 不存在 → deny（build 完必先過 verify）；②指令缺 `--draft` 或缺 `--assignee @me`（值須字面 `@me`，指派別人不算）→ deny 附完整補救指令；③slug 匹配 `^(\d+)-`（issue 編號開頭）時，PR body（`--body`/`--body-file`，重用 `extractCommentBody`＋`makeHardenedReadFileSafe`，經 `stripCode` 去 code span/fence 後）沒有獨立一行、行首純文字 `Closes #<issue#>` → deny（**刻意比 GitHub 實際解析更嚴**的 house rule：換版型一致＋零解析歧義；design slug 無數字前綴則跳過此閘、①②仍照常）；抽不到 body（無 body 參數／讀檔失敗）此閘 fail-open 放行，同 outbound-comment-guard 慣例。非 `gh pr create`／非 loop 分支／判不出分支一律放行；確需繞過設 `LOOPS_PR_GATE=0`。出錯 **fail-open（放行）**。footprint：無持久檔（純讀 `.git`／`.loops`，不寫任何檔案）。**已知限制**：① 與 outbound-comment-guard 同攔 `gh pr create --body` 時的多 hook 依序 deny 合併顯示行為 UNVERIFIED（Claude Code 無公開文檔；已拍板接受序列 deny——最壞情況是修一項再撞下一項、訊息各自清楚，見 issue #132 plan D3）；② 只認字面 `gh pr create`（正則解析、不跨 `; & |` 邊界），其他開 PR 途徑（web UI／API 直呼）不受管；③ 讀 `.git` 只看 cwd 本身、不像 `findLoopRoot` 那樣上溯祖先——payload.cwd 就是 Bash 呼叫當下的實際目錄，此假設成立；④ detached HEAD（裸 SHA，如 rebase 中）一律判不出分支、安全放行。
> - **`LOOPS_PR_BLOCKING_GATE`**（pr-gate 閘⑥，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#188**，**下界 #211 由 P0/P1 放寬成 P0**）：與 pr-gate 既有①–⑤ 同一支 hook、同一套 loop 分支判定，機械化 `iterate` §5「**收圈的機械下界＝P0 清零**」。在 loop 分支上 `gh pr create`／`gh pr ready` 時，讀 `stages/04-verify.md` 的機械 marker `<!-- loops-verify verdict=ready|not-ready p0=<n> p1=<n> round=<n> -->`（多輪 append 時最近一輪 wins、`matchAll` 無行錨 CRLF 安全、`round` 僅供人讀不用於選取）。**判定 fail-safe：raw 與 fence-robust stripped 兩視圖各取最後一個 marker、任一 blocking 即擋**——stripped 視圖擋「報告裡貼的 fenced 示範 ready marker 蓋掉真 not-ready」（原始 P2），raw 視圖擋「報告裡的 fence（貼的 code/diff、`` ``` `` 與 `~~~` 混用、未閉合 fence）把真 marker 藏進 fence 內、stripped 視圖漏讀 → 誤放行」（P2 的第一版修正引入、被 re-verify 抓到的反向漏洞；真 marker 一定在 raw、藏不掉）。**`p0` 權威（#211）**：`p0` 解得出數字時**它就是判定**——`p0>0` 擋、`p0=0` 放行（即使 `verdict=not-ready`、即使 `p1>0`；`verdict=ready p0=2` 這種矛盾仍擋，p0 贏）。`p0` **不是非負數字**（欄位缺 / 非數字 / 負數這種壞資料）時才退回 `verdict==='not-ready'`（嚴格全等、絕不 `includes`——`not-ready` 內含 `ready`）當保守訊號 → 擋（負數要走這條：把 `-1` 當合法計數會讓 `-1 > 0` 靜默放行、連兜底都問不到）。**`p1` 完全不參與判定**（仍要求 verify 如實寫，供人讀與 iterate 停損檢查點用）。仍有未修 P0 時：**auto 模式一律 deny**（`isAutoMode`＝`env.LOOPS_AUTO==='1'` 或 loop.md field-anchored「推進模式：auto」欄位行命中，**忽略 waiver**、防用豁免繞過、對齊 auto 硬煞車 #4）；**非 auto** 則有非空 `.loops/<slug>/blocking-waiver.md`（知情豁免留痕）才放行、否則 deny。**排在閘④ 前**（verify blocking 比截圖 receipt 更根本）、閘⑤ spawn gh 之前（純讀檔）。作用範圍 create+ready（comment 不套）；`gh pr ready` 另已被 `pr-owner-guard` 全擋，本閘-on-ready 是縱深。**fail-open**：marker 缺／`stripCode` 後無命中／欄位無法解析／讀檔失敗一律放行——所以「每輪吐 marker」是 verify 的紀律（見 `skills/verify` §5），marker 缺則閘⑥ 形同關閉。確需繞過設 `LOOPS_PR_BLOCKING_GATE=0`。出錯 **fail-open（放行）**。footprint：無持久檔（純讀 `.loops`）。**已知限制**：① marker 由 verify（agent）寫，adversarial 省略 marker ＝ fail-open 放行——本閘咬的是「verify 誠實記了 not-ready、卻順勢開 PR」這條 honest-momentum 路徑（＝ #162 踩過的「plugin 有寫沒做到」），對抗性省略由 skill-text 治理；**同類殘餘（verify 修正輪 F1，P3）**：報告若同時 (a) 用 fence／`~~~` 混用把真 not-ready marker 藏進 fence、且 (b) 在其後放一個 fenced `verdict=ready` 示範 marker，raw 與 stripped 兩視圖會被一起騙過 → 誤放行。這要報告**違反 verify §5「marker 寫在判定段最末、獨立一行」**才成立（honest 單輪報告不會），與「對抗性省略」同屬「擁有 `04-verify.md` 的 agent 可繞過」邊界、不降低已揭露的安全底線；要結構性根除需改契約（marker 須是檔案最後一行 / 另出 machine-only `04-verdict.json`）——列為 follow-up 決策、非本票 blocking；② auto 偵測的**唯一防竄改訊號是 env `LOOPS_AUTO=1`**（hook 讀 session env、agent 動不了），loop.md 訊號是 best-effort（agent 擁有 loop.md、理論上可改欄位行規避）——正常 auto（env）路徑機械保證成立；③ waiver 只驗「非空一般檔」、無法辨識人寫還 agent 寫（同 `no-ui.md` 既有限制）——由「agent 不得代決」skill-text 治理 who-authors，機械層保證的是「auto 一律不認 + 要一個刻意的非空檔」。
> - **`LOOPS_PR_VALIDATION_GATE`**（pr-gate 閘⑦，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#209**）：與閘⑥ 同一支 hook、同一份 `stages/04-verify.md`、同一個 marker，但讀的是**不同的兩個欄位**——`findings=`（步驟 3 去重後的候選 blocking finding 條數）與 `validated=`（其中經 `finding-validator` 二輪確認的條數）。機械化 `verify` §3「每條候選 blocking 都要過二輪確認」。**為什麼要另立欄位而不是沿用 `p0`/`p1`**：後者是「判定當下**仍未修**」的條數，Ready 時必為 0，因此永遠回答不了「這一輪到底有沒有 finding、第二輪有沒有跑」——這正是第二輪被整段跳過卻無人察覺的機制（#209 實測：一條完整 loop 派了 10 個 reviewer、`finding-validator` **0 個**，4 條 finding 未經確認就驅動了 iterate）。**判定（放行是預設、deny 是例外）**：`findings>0 && validated===0` → deny；兩欄缺席（舊報告 / 舊版 verify）→ 放行（**缺席 ≠ 沒派**，把「不知道」當違規就是造假數字）；`findings=0` → 放行（零候選本來就不必派）。唯一 fail-safe 例外＝**已宣告 `findings>0` 卻沒宣告 `validated=`** → deny，它打不到舊報告（舊報告連 `findings` 都沒有）、只打得到半套契約，修法就是補上那個數字。與閘⑥ 同用 raw + fence-robust 兩視圖聯合、任一命中即擋。**不認 `blocking-waiver.md`**（與閘⑥ 最大的差異）：那份豁免的語意是「知情接受這些風險」，但二輪確認沒跑時風險**還沒被評估過**，沒有東西可以知情接受；因此 auto／attended 一視同仁，逃生口只有 `LOOPS_PR_VALIDATION_GATE=0`。排在閘⑥ 後、閘④ 前（純讀檔、與⑥ 共讀同一份報告不重複讀檔）。作用範圍 create+ready。出錯 **fail-open（放行）**。footprint：無持久檔。**已知限制**：marker 由 verify（agent）自己寫，`validated=` 是自述而非機械觀測——本閘咬的是「誠實記了 `findings=4 validated=0` 卻順勢收圈」這條路徑，對抗性填假數字由 skill-text 與 `scripts/verify-dispatch.mjs` 的事後觀測治理（該腳本從 transcript 數真實派工，但**數不到候選 finding 條數**，故對「派了 reviewer、沒派 validator」一律回 `unconfirmed`、不聚合成違規率——兩邊各補對方的盲區）。
> - **`LOOPS_PR_FOOTPRINT_GATE`**（pr-gate 閘⑧，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#215**）：與閘⑥⑦ 同一支 hook、同一份 `stages/04-verify.md`，但讀的是**另一種 marker**：`<!-- loops-footprint status=ok|warn|blocked prod=<n> test=<n> newtests=<n> unslotted=<n> unexplained=<n> -->`，由 `scripts/diff-footprint.mjs` 在 verify 步驟 0 產出、verify 原樣貼進報告。機械化 `AGENTS.md` 規則 17「未說明的 footprint drift 不得收圈」。**判定：只擋 `status=blocked` 一格**——四種成因（改動檔不屬於任何核准 slice／新測試缺 `new_test_reason`／同一 behavior 兩份 primary evidence 缺 `distinct_risk`／超出 change budget 又沒 `budget_overrun_reason`）。**`status=warn` 一律放行**：那是「測試新增行數多於功能新增行數」的提醒，**比例不是品質標準、永遠不當阻擋理由**——用固定 ratio 擋掉正當的測試，正是這條規則明列的非目標。marker 缺席 / 無法解析 → fail-open 放行（舊 loop 相容）。與閘⑥⑦ 同用 raw + fence-robust 兩視圖聯合、任一命中即擋。排在閘⑦ 後、閘④ 前（純讀檔）。作用範圍 create+ready。**不認 `blocking-waiver.md`**：那份豁免的語意是「知情接受這些 P0/P1」，與「範圍與證據跟計畫對不上」是不同的事；footprint 的出口是**回 plan 補 living plan**（納入 slice／補 `new_test_reason`／補 `distinct_risk`／補 `budget_overrun_reason`），不是刪 code 或刪測試湊數字。出錯 **fail-open（放行）**。footprint：無持久檔（純讀 `.loops`）。**已知限制**：marker 由 verify（agent）自己貼，省略 marker ＝ fail-open —— 同閘⑥⑦ 的既有邊界，由 skill-text 治理；腳本本身不 spawn git 以外的東西，且「量不到 diff 就據實回報量不到、不編數字」（規則 5）。
> - **`LOOPS_PR_EFFORT_GATE`**（pr-gate 閘⑨，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#222**）：與閘⑥⑦⑧ 同一支 hook、同一份 `stages/04-verify.md`，讀的是**第三種 marker**：`<!-- loops-effort profile=direct|standard|deep floor=ok|violated highrisk=yes|no|unknown escalated=<n> -->`，由 `scripts/effort-profile.mjs --audit` 在 verify 步驟 0 產出、verify 原樣貼進報告。機械化 `AGENTS.md` 規則 25 的地板。**判定：只擋 `floor=violated` 一格**——loop 宣稱走最省的 `direct` 檔位，實際改動卻碰到高風險硬閘路徑（auth／密鑰／金流／schema·migration／對外契約／並發背景／IaC）。`direct` 的第一條判準就是「不碰高風險硬閘」，判準不成立還照 direct 跑完，等於用省成本的名義跳過該做的驗證。**`highrisk=unknown`（量不到 diff）→ 腳本本來就寫 `floor=ok`，所以放行**——判不出來不等於違規；marker 缺席 / 無法解析同樣 fail-open 放行（舊 loop 相容）。與閘⑥⑦⑧ 同用 raw + fence-robust 兩視圖聯合、任一命中即擋。排在閘⑧ 後、閘④ 前（純讀檔）。作用範圍 create+ready。**不認 `blocking-waiver.md`**：那份豁免的語意是「知情接受這些已評估的風險」，而檔位判錯時該做的驗證根本還沒跑過，沒有東西可以知情接受。出口是**升檔補做**（`R-high-risk`，見 `references/stages/effort-profile.md` §E），不是把 marker 改掉。出錯 **fail-open（放行）**。無持久檔（純讀 `.loops`）。**已知限制**：marker 由 verify（agent）自己貼，省略 marker ＝ fail-open —— 同閘⑥⑦⑧ 的既有邊界，由 skill-text 治理。
> - **`LOOPS_MERGE_GUARD`**（merge-guard，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection -->；**預設開——僅字面 `'0'` 關**；**#133**）：與 pr-gate 是姊妹規則（那管「開 PR 前要過的閘」、本 hook 管「合併這個動作本身要人核可」）——**不限 loop 分支**，任何 cwd 偵測到以下四型「合併回主幹」指令一律 deny（依序判定，命中即擋，不聚合）：①`gh pr merge`（任意 flag 組合）；②cwd 目前所在分支是 main/master 時的 `git merge <ref>`（讀 `.git`，重用 pr-gate.mjs 匯出的 `readGitBranch`——檔案形/目錄形 HEAD 皆生效；detached HEAD 或判不出分支一律放行，非主幹分支上的 `git merge`〔互併 feature 分支〕合法不擋）；③`git push` 的目的地是 main/master（bare positional／refspec 冒號右側／`--delete` 皆算，**`--delete` 不豁免**；push 到 feature 分支放行）；④`gh api` 用 `-X PUT`／`--method PUT` 打 `/pulls/.../merge` 路徑（GET 或非 `/merge` 路徑放行）。**視圖分工**（同 #132 Q1 課＋審查實測補強兩缺口）：子指令詞判定（是不是 `gh pr merge`／`git merge`／`gh api`+PUT）用 `stripQuotedValues` 剝殼視圖判，避免字樣只出現在別的指令的引號值裡（如 comment body）被誤判；但 push 目的地與 api 路徑改判**原始未剝殼字串**——剝殼視圖會把引號包住的目的地/路徑一併消掉（`git push origin "HEAD:master"` 整段 refspec 被引號包住、`gh api -X PUT "repos/x/y/pulls/1/merge"` 路徑被引號包住這兩形），對剝殼視圖判會造成偽陰性、漏放真正的高風險指令。非四型指令、四型指令的目的地/路徑不中一律放行；確需繞過設 `LOOPS_MERGE_GUARD=0`。出錯 **fail-open（放行）**。footprint：無持久檔（純讀 `.git`，不寫任何檔案）。**已知限制**：① 只攔字面型指令（正則解析 `gh pr merge`／`git merge`／`git push`／`gh api` 四種固定形態），`git rebase`／`git cherry-pick` 等其他能把 commit 帶進主幹的途徑不在此列（暫不擴大範圍）；② 與 outbound-comment-guard／pr-gate 同攔同一條指令時的多 hook 依序 deny 合併顯示行為 UNVERIFIED（同 #132 已拍板接受序列 deny——最壞情況是修一項再撞下一項）；③ 只認得這四種指令形態，其他合併途徑（web UI／未列出的 API 端點）不受管；④ push 目的地判定（#133 verify 二次仲裁後改 token 化 positional 解析、丟棄 flag token，取代舊版「只看字串最後一個 token」）已大幅收斂 refspec／選項值誤判（如 `--push-option="note:master"` 這種值裡湊巧含冒號+分支名圖樣不再誤中），殘餘面：以空白分隔、被引號包住的 positional 一律視為目的地候選、不論其實際語意是不是某個 flag 的值，仍可能被保守誤判為目的地；⑤ `git push`（無任何位置引數，依賴 tracking 分支設定推）不解析 `remote.origin.*`／`branch.*.merge` 等 git config 判斷實際目的地，一律放行（fail-open 精神延伸：判不出目的地就不擋）。**與使用者層既有規則的關係**：本 hook 與使用者層（如 `settings.json` 自訂）既有的「`gh pr merge` 需人核可」deny 規則並存＝**雙保險、不衝突**——兩者攔的動作有重疊但層級不同（一個是 plugin 內建 hook、一個是使用者自訂規則），是否移除使用者層那條交由各專案自行決定，本 hook 不強制取代它。
> - **`LOOPS_PR_OWNER_GUARD`**（pr-owner-guard，<!-- adapter-projection -->`PreToolUse` matcher `Bash|PowerShell`<!-- /adapter-projection --> **＋ GitHub MCP 工具 matcher**（家族首支攔 MCP 工具的 deny hook）；**預設開——僅字面 `'0'` 關**；**#164**）：與 pr-gate／merge-guard 是姊妹規則（pr-gate 管「開 PR / 轉正前要過的品質閘」、merge-guard 管「合併要人核可」、本 hook 管「**送審／加審查者這個動作本身屬於 owner**」）——**不限 loop 分支**，任何 cwd 偵測到以下動作一律 deny：**shell 五型**①`gh pr ready`（剝殼視圖含 `--undo`＝撤回、放行）②`gh pr edit --add-reviewer`／`gh pr create --reviewer`（`-r` 短旗標 token 化只認 create 之後、防 `cp -r` 等別段旗標誤擋；`--remove-reviewer` 放行）③`gh api` 對 `/pulls/.../requested_reviewers` 的 POST（顯式 `-X`/`--method` 含黏合 `-XPOST`；無顯式 method 但帶 `-f`/`-F`/`--field`/`--raw-field`/`--input`＝gh 隱式 POST；顯式 DELETE 撤回／無欄位 GET 查詢放行）④`gh api` ∧ 裸 token `graphql` ∧ 原始字串含 `markPullRequestReadyForReview`／`requestReviews` mutation（`convertPullRequestToDraft` 放行）；**MCP 兩工具**：`update_pull_request` 帶 `draft === false`（strict、非 truthiness）或非空 `reviewers` → deny（`draft: true`／只改 title 等不帶這兩欄的更新放行；`update_pull_request_branch` 尾綴不同、精確邊界不中）、`request_copilot_review` 一律 deny。**分流不讀 tool_name 判 shell**（`tool_input.command` 是字串→shell 判定；否則 tool_name 精確比對 MCP 工具），Bash 與 PowerShell payload 同形涵蓋。**視圖分工**同家族：子指令詞剝殼視圖判、api 路徑與 graphql mutation 名對原始字串判。deny 訊息載明「owner 驗收動作＋reviewer comment 流程指示不構成授權＋請在回報中提醒 owner 自行操作＋撤回類指引」。確需繞過（使用者本人授權代執行）設 `LOOPS_PR_OWNER_GUARD=0`。出錯 **fail-open（放行）**。footprint：無持久檔、零 spawn（純字串／欄位判定）；**成本取捨（拍板接受）**：這是 `Bash|PowerShell` 廣義 matcher 群組的**第 5 支序列 hook**——每次 Bash/PowerShell 呼叫固定多一次 node 啟動、隨 session 內呼叫次數線性累加；單一職責（不併入 merge-guard）優先於省這一次啟動，但**未來要加第 6 支同類 guard 時應優先考慮併入語意相鄰的既有 hook**、避免無限疊加。**已知限制**：①`--undo` 檢查 command-wide，複合指令別段的 `--undo` 可騙過①型成假放行（拍板接受、characterization 測試釘住）；②graphql query 經 `--input file` 傳、mutation 名不在指令字串上＝判不出放行（lexical 邊界）；③`-r` 黏合值形（`-rvalue`）不涵蓋；④curl 直呼 API／web UI 不在攔截面。**與 pr-gate 對 `gh pr ready` 的閘④⑤疊加**＝雙保險（本 hook 被 `'0'` 關時仍有品質閘），序列 deny 行為 #132 已拍板接受。

> **評測 hook（#35 + #49 擴成多訊號，永不擋路；#87 起三 flag 皆預設開——僅字面 `'0'` 關、無 artifact 即 no-op 零噪音、不執行 repo 定義命令）**：
> - **`LOOPS_EVAL_GATE`**（eval-gate，<!-- adapter-projection -->Stop hook + edit-accumulator `PostToolUse`<!-- /adapter-projection -->；預設開）：**本回合有改檔**且 cwd 有 `.loops/.metrics/eval-results.jsonl` 時，於 Stop 自動跑 `eval-metrics.mjs check`，**只有偵測到 passRate 退化（exit 1）才把警示注入 context**（無退化靜默、不阻擋）。與 stop-gate 共用 edit-accumulator：在 hooks.json 排其**前**、**僅 stop-gate 未啟用時才自清** accumulator（避免兩 gate 互踩）。footprint：`os.tmpdir()/loops-edits-<session>.json`（與 stop-gate 共用）。
...
auto 模式（見 `references/shared/runtime/auto-mode.md`）暫停時，journal 記「auto 因 X 暫停於 E_n」；resume 時從該點接續，不重跑已完成的階段。

## 為什麼 append-only

- 保留完整決策軌跡（誰在哪個 gate 選了什麼、為什麼回環）—— 事後可稽核、可回溯。
- 不覆蓋 = 不會因為改寫遺失「為什麼走到這」。
- 也可讓計畫檔額外帶一塊「可被腳本檢查」的結構，做到 plan → validate → execute（見 `references/stages/machine-plan-schema.md`）。
