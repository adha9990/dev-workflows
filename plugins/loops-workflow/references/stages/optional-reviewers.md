...
> verify 的核心 reviewer（product-contract / architecture / security / performance / code-quality / tests）覆蓋每次都該看的主軸 —— **派哪幾軸由 verify 步驟 1 決定**：有 risk map 時走 `risk-map.md` §C（**固定 `product-contract` + `code-quality`，其餘依 predicate 觸發**）；高風險硬閘命中一律 6 軸滿派；沒有 risk map 則退回 `verify-triage.md` 的 0~6 風險梯。本檔是領域加派層〔conditional〕，與核心軸的選法**正交**。下面這些是**領域特定**的額外視角 —— **只在改動觸及該領域時才派**，避免無關維度造成噪音。觸發時與核心 reviewer **同一回合一起派**（並行）。
...
- 核心 6 軸（範圍契約 / 架構 / 安全 / 後端效能 / 程式品質〔含 correctness〕/ 測試）：**派哪幾軸由 verify 步驟 1 決定** —— 有 risk map 走 `risk-map.md` §C（固定範圍契約 + 程式品質，其餘依 predicate）；高風險硬閘一律 6 軸滿派；沒有 risk map 退回 `verify-triage.md` 風險梯（一般 code 6 軸、小孤立 3 軸、瑣碎 0）。**右尺寸化只依風險浮動、不是給 code 開後門**（拿不準 / 混 code / 碰高風險 path 一律向上升級）。
