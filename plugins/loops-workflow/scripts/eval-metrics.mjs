...
// 防惡意巨檔 DoS（#87 defaultOn 後自動吃 repo 可控輸入，security review）：readEvalRows 讀檔前的
// 位元組上限，與 MAX_ORACLE_STDOUT 同款量級，避免對 repo 控制的 eval-results.jsonl 無界讀入。
 ...
 * maxBytes（可選，預設 MAX_INPUT_FILE_BYTES）：讀前先 statSync 檔大小，超限直接回 []（安全空值，
 * 不讀入超限內容）——防惡意巨檔 DoS（#87 defaultOn 後自動吃 repo 可控輸入）；永不擋路、永不拋錯。
 */
export function readEvalRows(file, maxBytes = MAX_INPUT_FILE_BYTES) {
  try {
    if (statSync(file).size > maxBytes) return []; // 超限 → 安全空值，不讀入內容
  } catch {
    return []; // 檔不存在 / stat 失敗 → 空歷史
  }
  let content;
  try {
    content = readFileSync(file, 'utf8');
  } catch {
    return []; // 讀不到 → 空歷史（沒資料不等於錯誤）
  }
