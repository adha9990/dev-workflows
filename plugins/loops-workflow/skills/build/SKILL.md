...
5. **Refactor**（綠燈後、test 保護下整理結構不改行為）：派 impl-author 時把 `references/shared/quality/refactoring.md` 與 `references/shared/quality/code-simplification.md` 的**絕對路徑**寫進其 prompt（subagent 用相對路徑讀不到，見 AGENTS.md〈參考檔路徑解析〉）—— **`refactoring`：先對到一個具名 code smell 才動、用具名手法（Extract Function / Replace Conditional with Polymorphism…）小步改、設計模式對症才引入**；**`code-simplification`：Chesterton's Fence、過度簡化四陷阱、紅旗「簡化若需要改 test 才能過 = 你改的是行為不是結構，停下」**。
