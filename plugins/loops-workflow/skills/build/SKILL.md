...
1. **派 `test-author`**：只給它需求 / 契約 + TDD 品質判準，**它的 context 不含 implementation**；把 `references/test-rubric.md` 的**絕對路徑**寫進其 prompt（分層測試 unit/integration/smoke/e2e、real-not-mock、async 等真完成、data-layer 覆蓋清單；subagent 用相對路徑讀不到）。**派之前先讀 `loop.md` 的 `operation` 性質，依 `references/operation-first-move.md` 把對應「紅燈第一步規則」併入 test-author 的 prompt**：`bug-fix`→第一個測試必須**重現 bug**（fix 前必失敗）／`change-behavior`→先把**既有相關測試改成新規格**讓它失敗／`refactor`→先確認既有測試**全綠**、無覆蓋先補 characterization test、**本輪不寫新行為測試**／`new-feature`（含拿不準的 fail-safe）→標準 TDD。此規則只影響紅燈起手式，不改紅綠分離、不破壞「test-author 不見 impl」。它回 failing test + 「這測哪條需求」。
