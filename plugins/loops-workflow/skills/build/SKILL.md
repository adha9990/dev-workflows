...
4. **主線跑測試 → 確認 Green**。
5. **Refactor**（綠燈後、test 保護下整理結構不改行為）：套 `code-simplification`（見 `references/code-simplification.md`）—— Chesterton's Fence（改 / 刪前先答「為什麼當初這樣寫」）、過度簡化四陷阱、**紅旗「簡化若需要改 test 才能過 = 你改的是行為不是結構，停下」**。
