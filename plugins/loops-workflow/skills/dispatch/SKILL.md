...
動工前先把「**我理解你要什麼 + 關鍵假設 + 不做什麼**」用一兩句**複述**給使用者，並用 `AskUserQuestion` 做**一次**確認 —— 工程師常用一句話描述需求，理解一旦偏掉，後面整條 loop 會持續往錯方向做、越做越貴（規則 10 成本意識）。**特別是**：一句話 / 模糊需求、greenfield 新專案、範圍不明時**必做**；已經很明確的情境（顯式 `dispatch <type> <ref>`、issue# 帶完整描述、單純轉場）可略過。確認對齊後再進第 1 步判類型。

> 這是**便宜的前置對齊**（單次確認），不是反覆訪談 —— 細部完工定義仍由 `goal` 階段釘。
