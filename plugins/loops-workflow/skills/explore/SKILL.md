...
| 維度 | 看什麼 |
|------|------|
| **效能** | 時間 / 空間複雜度、可預見規模（資料量 / 流量）下的表現；能 micro-benchmark 就量 |
...
主線收成**比較矩陣**（候選 × 維度 + 分數 + 證據 / CITE，沒實測標 `not measured`）→ 寫進 `01-explore.md` → 推薦 + **點明哪個維度是決定因素**。**單一明顯方法、無真競爭** → 跳過評估直接推薦（省成本）。
