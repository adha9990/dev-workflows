...
> **補寫 `operation`（僅 bypass 入口需要）**：正常經 dispatch 進來時 `loop.md` 已有 `operation` 欄（dispatch 寫）。但若使用者**直接喊 `/loops-workflow:goal`（未經 dispatch）**導致 loop.md 沒有 `operation` 欄 → 順手依 issue 性質判定並補寫（`new-feature` / `change-behavior` / `bug-fix` / `refactor`，見 `references/operation-first-move.md`；拿不準向嚴 `new-feature` 並在 Journal 註明）。這欄決定 build 紅燈第一步，缺了 build 會退到 `new-feature` 預設。
