...
> **先讀 `loop.md` 的投入檔位**（`AGENTS.md` 規則 25）。本階段是固定 ceremony 最集中的地方，所以檔位在這裡差別最大——`direct` 出**一頁 micro-plan**（現況一段 → slice 含 `files` 與 change budget → evidence → 決策若有）、免機制圖、對齊 comment 出精簡版；`standard` 出完整 §0–§9；`deep` 再加 ADR 與契約段。**縮的是文件體積與審查輪數，不是步驟**——每一步該做的判斷照做，`loops-plan` 區塊、change budget、`plan → build` 拍板 gate、設計審查必派**任何檔位都不動**。逐項對照表是 `references/stages/effort-profile.md` §C，**此處不重抄**。檔位在 plan 期間冒出新機制／新套件／新對外契約／blocking unknown → **當下升檔並補做**（同檔 §E），不是「這次先這樣」。
...
### 3. 機制圖（每機制：白話 + 兩張圖；`direct` 檔位免——但只在「真的沒有新機制」時）

**`direct` 的判準 `D3` 就是「沒有新架構接縫」**，所以它通常沒有機制可畫。**冒出任何一個新機制 → 升檔到 `standard` 並把圖補上**，不是「這個機制很小、口頭說明就好」。其餘檔位照下面做。
...
- **設計品質審查（plan 前先 verify）—— 一律必派，但強度依風險分級**：**每一次 plan 都必派 read-only agent 審 `stages/02-plan.md`**（跳過設計審查、憑未查證的假設就拍板，正是過去反覆出包的根因；**必派這件事沒有例外**）。審查對設計做「六維度 + 落點對齊（對照實檔 file:line）+ 契約」，出「方向可行 / 要修 / 資訊不足」判定 —— 把方向 / 落點錯擋在 code 之前，別等 build 完才在 verify 發現方向就錯。
  - **強度依 risk map ＋ 投入檔位分級（改的是「多深、審幾輪、餵多少輸入」，不是「派不派」）**：
    ...
    - **`direct` 檔位**：一輪，輸入是**一頁 micro-plan ＋ 落點檔**，用 `fast-readonly` tier（`references/shared/runtime/model-effort-policy.md`）。**這是「餵它更少東西」，不是「不派它」**——`direct` 沒有系統全貌可審，但落點對不對、有沒有現成的可以重用、change budget 合不合理，一頁就審得完。**曾經考慮過在 `direct` 直接免派，被否決**：跳過設計審查、憑未查證的假設就拍板，正是過去反覆出包的根因；而它在 `direct` 的邊際成本本來就很低（輸入只有一頁）。
...
**在 plan 階段就把計畫草稿送出**（不是等 loop 結束）：issue-driven → 依 **`skills/plan/references/plan-comment-template.md`（本 skill 目錄下；完整版：系統全貌 + 套件清單含版本 + ADR + 機制圖 + 施工圖 + 契約 + out-of-scope）**（**`direct` 檔位改出精簡版：要改什麼 / 怎麼驗 / 已知風險三段、≤15 行**——它沒有系統全貌與機制圖可放，硬套完整版只會生出一堆「無」）寫暫存 tmp 草稿校稿後 post 成 issue 對齊 comment（留 audit trail，**post 後刪 tmp**；更新既有 comment 用 `gh api --method PATCH repos/<owner>/<repo>/issues/comments/<id> -F body=@<tmp>`）；非 issue → 呈現給使用者。**這則 comment 是 living as-built 摘要**，build 偏離時回來同步更新（含已 post 的版本）。
...
| 「這條 loop 是 `direct`，設計審查就免了吧」 | `direct` 縮的是**餵給它多少東西**（一頁 micro-plan ＋ 落點檔、`fast-readonly` tier），不是派不派。地板列得很清楚：設計審查必派，沒有檔位例外。 |
| 「`direct` 也把 §0–§9 補齊比較完整」 | 為一個兩檔五十行的改動寫一份系統全貌，生出來的是一份沒人會讀的文件——這正是這條 loop 想省掉的成本。真的需要那份全貌，代表它不是 `direct`，該升檔。 |
| 「做到一半發現碰到 migration，先做完再說」 | 那是 `R-high-risk`，**當下升檔**並補做新檔位多的那些 knob。帶著錯的檔位跑完，PR 前會被閘⑨ 擋下來，而且那時要補的更多。 |
...
- **`direct` 檔位卻照樣產完整 §0–§9 施工圖 / 完整版對齊 comment**（固定 ceremony 沒有隨檔位縮＝這條規則等於沒生效），或反過來**`direct` 免派設計審查 / 不寫 `loops-plan` 區塊與 change budget**（那是地板，不是 knob）。
- **plan 期間冒出新機制 / 新套件 / 新對外契約 / blocking unknown 卻沒升檔**（帶著錯的檔位往下走，PR 前才被閘⑨ 擋）。
...
- [ ] **已讀 `loop.md` 的投入檔位，產物體積與它對得上**（`direct`→micro-plan＋精簡對齊 comment＋免機制圖；`standard`→§0–§9；`deep`→＋ADR/契約段）；**地板沒有隨檔位縮**（設計審查已派、`loops-plan` 與 change budget 已寫、拍板 gate 有停）。
- [ ] 期間若冒出新機制 / 新套件 / 新對外契約 / blocking unknown，**已當下升檔並補做**新檔位多出來的 knob（`loop.md` 欄位已改 ＋ Journal 有一筆 `舊 → 新｜觸發 id｜證據`）。
