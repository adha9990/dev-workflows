...
- **每張圖下方必附 1–2 句白話「圖說」**：講這張圖在做什麼、讀者該看出什麼——GitHub 會把圖畫出來，但**光丟圖、旁邊沒白話說明，讀者看不懂**。圖說緊貼在該圖下面（不是只在別段落的系統全貌裡帶過）。範式：` ```mermaid ``` ` 之後接一行 `**圖說**：<白話一句這張圖在幹嘛>`。（實測教訓：曾把三張機制圖貼進 comment 卻沒在每張下面附白話，使用者當場打回「機制圖底下應該要有白話說明再幹嘛」。）
