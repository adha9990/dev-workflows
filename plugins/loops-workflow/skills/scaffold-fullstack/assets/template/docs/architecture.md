...
這份文件給新人一個系統全景:怎麼分層、有哪些不能跨的牆、擴充時照什麼規則走。動程式碼前先讀這裡。
...
**不變式**:client 不直接讀資料庫、後端不 import 前端的型別 —— 這道牆由 **workspace 的 package 邊界**
保證(`backend/` 與 `frontend/` 不在彼此的相對路徑內,根本沒有跨界 import 的途徑,不需要靠 ESLint 規則
去擋);為的是讓後端能獨立部署(standalone、Docker、或嵌入 Electron host)、前端維持純 client。
...
**擴充時的判斷(ports + adapters)**:內層依賴*介面*而非實作 —— 要換基礎設施(如 SQLite → Postgres)
就寫一個新 adapter,repositories 與 services 完全不動;測試時注入假的 store,不 mock 內層。唯一知道
真實接線的地方是 `backend/src/bin/server.ts`。
...
**約束**:View 只認得 viewmodel 暴露的 `{ data, status, actions }`,碰不到 fetch / DTO 細節 ——
換資料來源或調呈現邏輯只動 viewmodel(與後端 ports/adapters 同一種精神:把不穩定的細節關在內層)。
...
**不這樣做會踩的坑**:dev / CI / production 的 schema 漂移;型別與 schema 失同步時,照這條流程走
TypeScript 會立刻報錯,而不是等到 runtime。
...
- 測試怎麼分、加新測試怎麼選:[`testing.md`](./testing.md)
