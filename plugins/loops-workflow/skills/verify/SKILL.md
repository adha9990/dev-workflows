...
- **跑真 app**：**先查目標專案的 `AGENTS.md`/`CLAUDE.md` 有沒有宣告專用的 run / verify / smoke skill 或啟動方式**（很多專案自帶「啟動並驅動這個 app」的 skill）—— 有就**優先用它實跑**，把宣稱從 `not measured` 變實測，別跳過；沒有才退到通用 `/run` 起服務 + `/verify` 逐條玩 `00-goal.md` 需求 + 本機 `/code-review`（**不跑 ultra 計費版**）。純 lib 無 app 才據實標 `not measured`；宿主環境連這類 skill 都沒有時，以手動啟動指令替代或據實標 `not measured`。**有可跑的 app 卻直接標 `not measured` 是偷懶**——專案宣告了怎麼跑就要跑。跑測試／app 的輸出收斂依 `references/context-diet.md` §A（紅綠不對稱、截斷附落盤路徑）。
